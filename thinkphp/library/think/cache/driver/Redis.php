...
class Redis extends Driver
{
     ...
     * @return boolean
     */
    public function push($name,$value,$right='r'){
        $key = $this->getCacheKey($name);
        $value = (is_object($value) || is_array($value)) ? json_encode($value) : $value;
        if ($right=='r'){
     ...
     * @param string $right 取出方向
     * @return string|unknown|mixed
     * ***/
    public function pop($name,$default=FALSE,$right='r'){
        $key = $this->getCacheKey($name);
        if ($right=='r'){
    ...
    public function hMget($name,$default=FALSE){
        $value = $this->handler->hMget($this->getCacheKey($name));
        if (is_null($value) || false === $value) {
            return $default;
        }
        $jsonData = json_decode($value, true);
    ...
    public function sMember($name,$default=FALSE){
        $value = $this->handler->sMembers($this->getCacheKey($name));
        if (is_null($value) || false === $value) {
            return $default;
        }
        $jsonData = json_decode($value, true);
